...
test("Test GithubRepoLoader", async () => {
    ...
    expect(documents.filter((document) => document.metadata.source === "yarn.lock")
        .length).toBe(1);
    expect(documents.filter((document) => document.metadata.source === "README.md")
        .length).toBe(1);
...
test("Test ignorePaths with GithubRepoLoader", async () => {
    ...
    expect(documents.filter((document) => document.metadata.source === "yarn.lock")
        .length).toBe(0);
    expect(documents.filter((document) => document.metadata.source.endsWith(".md"))
        .length).toBe(0);
    console.log(documents[0].pageContent);
});

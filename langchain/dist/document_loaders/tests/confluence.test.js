...
test("Test ConfluenceLoader and fetchConfluenceData calls", async () => {
    ...
    const fetchConfluenceDataMock = jest
        ...
        .mockImplementationOnce(() => Promise.resolve({ size: 2, results: fakeResponse }))
        .mockImplementationOnce(() => Promise.resolve({ size: 2, results: fakeResponse }))
    ...
    expect(fetchConfluenceDataMock).toHaveBeenNthCalledWith(1, "https://example.atlassian.net/wiki/rest/api/content?spaceKey=SPACEKEY&limit=25&start=0&expand=body.storage");
    expect(fetchConfluenceDataMock).toHaveBeenNthCalledWith(2, "https://example.atlassian.net/wiki/rest/api/content?spaceKey=SPACEKEY&limit=25&start=2&expand=body.storage");
    expect(fetchConfluenceDataMock).toHaveBeenNthCalledWith(3, "https://example.atlassian.net/wiki/rest/api/content?spaceKey=SPACEKEY&limit=25&start=4&expand=body.storage");

...
test("Test JSON loader", async () => {
    const filePath = path.resolve(path.dirname(url.fileURLToPath(import.meta.url)), "./example_data/Star_Wars_The_Clone_Wars_S06E07_Crisis_at_the_Heart.json");
...
test("Test JSON  loader for complex json without keys", async () => {
    const filePath = path.resolve(path.dirname(url.fileURLToPath(import.meta.url)), "./example_data/complex.json");
    const loader = new JSONLoader(filePath);
    const docs = await loader.load();
    ...
    expect(docs[2]).toEqual(new Document({
...
test("Test JSON loader for complex json with two keys that exists", async () => {
    ...
    expect(docs[3]).toEqual(new Document({

...
import { HumanMessagePromptTemplate, SystemMessagePromptTemplate, ChatPromptTemplate, } from "../../prompts/chat.js";
import { ChatOpenAI } from "../../chat_models/openai.js";
import { LangChainTracer } from "../../callbacks/index.js";
import { FewShotPromptTemplate, LengthBasedExampleSelector, } from "../../prompts/index.js";
...
test("serialize + deserialize llm with optional deps", async () => {
    ...
    const str = JSON.stringify(llm, null, 2);
    expect(stringify(JSON.parse(str))).toMatchSnapshot();
    const llm2 = await load(str, { COHERE_API_KEY: "cohere-key" }, { "langchain/llms/cohere": { Cohere } });
    expect(llm2).toBeInstanceOf(Cohere);
    expect(JSON.stringify(llm2, null, 2)).toBe(str);
    const llm3 = await load(str, { COHERE_API_KEY: "cohere-key" }, { "langchain/llms/cohere": import("../../llms/cohere.js") });
...
test("serialize + deserialize llm chain few shot prompt w/ examples", async () => {
    ...
    await expect(load(str, {
        OPENAI_API_KEY: "openai-key",
    })).rejects.toThrowError('Trying to load an object that doesn\'t implement serialization: $.kwargs.prompt -> {"lc":1,"type":"not_implemented","id":["langchain","prompts","few_shot","FewShotPromptTemplate"]}');
});
test("serialize + deserialize llm chain few shot prompt w/ selector", async () => {
    ...
    await expect(load(str, {
        OPENAI_API_KEY: "openai-key",
    })).rejects.toThrow('Trying to load an object that doesn\'t implement serialization: $.kwargs.prompt -> {"lc":1,"type":"not_implemented","id":["langchain","prompts","few_shot","FewShotPromptTemplate"]}');
});

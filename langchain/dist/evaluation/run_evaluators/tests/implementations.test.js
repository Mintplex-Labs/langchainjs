...
import { ChoicesOutputParser, StringRunEvaluatorInputMapper, getCriteriaEvaluator, getQAEvaluator, } from "../implementations.js";
...
test("Test getCriteriaEvaluator with correct input", async () => {
    class FakeLLM extends LLM {
        ...
        async _call() {
            return ("It's fairly concise and correct I'd reckon, " +
                "but I'm still going to fail you.\n\nN");
        }
    }
    const model = new FakeLLM({});
    const chain = await getCriteriaEvaluator(model, ["correctness", "conciseness"], {
        inputKey: "query",
    ...
    expect(result).toEqual({
        ...
        score: 0,
        value: "N",
        comment: "It's fairly concise and correct I'd reckon, but I'm still going to fail you.",
    });
});

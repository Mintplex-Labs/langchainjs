...
test("Test OpenAI with stop", async () => {
    ...
    const res = await model.call([new HumanChatMessage("Print hello world")], ["world"]);
    console.log({ res });
});
...
test("Test OpenAI with timeout in call options", async () => {
    const model = new ChatOpenAI({ maxTokens: 5 });
    await expect(() => model.call([new HumanChatMessage("Print hello world")], {
        options: { timeout: 10 },
    })).rejects.toThrow();
}, 5000);
...
function createSampleMessages() {
    ...
    return [
        ...
        new HumanChatMessage("This late pivot means we don't have time to boil the ocean for the client deliverable."),
    ];
}

...
import { getPromptTemplateFromDataSource, verifyIgnoreTablesExistInDatabase, verifyIncludeTablesExistInDatabase, } from "../sql_utils.js";
...
test("Find include tables when there are there", () => {
    ...
    expect(() => verifyIncludeTablesExistInDatabase(allTables, includeTables)).not.toThrow();
});
test("Throw Error when include tables are not there", () => {
    ...
    expect(() => verifyIncludeTablesExistInDatabase(allTables, includeTables)).toThrow();
});
test("Find include tables when there are there", () => {
    const includeTables = ["user", "score"];
    const allTables = [
    ...
    expect(() => verifyIgnoreTablesExistInDatabase(allTables, includeTables)).not.toThrow();
});
test("Throw Error when include tables are not there", () => {
    const includeTables = ["user", "score"];
    const allTables = [
    ...
    expect(() => verifyIgnoreTablesExistInDatabase(allTables, includeTables)).toThrow();
});

...
test("Test HNSWLib.fromTexts + addVectors", async () => {
    const vectorStore = await HNSWLib.fromTexts(["Hello world"], [{ id: 2 }], new FakeEmbeddings());
...
test("Test HNSWLib metadata filtering", async () => {
    ...
    const vectorStore = await HNSWLib.fromTexts([pageContent, pageContent, pageContent], [{ id: 2 }, { id: 3 }, { id: 4 }], new FakeEmbeddings());

...
test("Test FaissStore.fromTexts", async () => {
    const vectorStore = await FaissStore.fromTexts(["Hello world", "Bye bye", "hello nice world"], [{ id: 2 }, { id: 1 }, { id: 3 }], new OpenAIEmbeddings());
...
test("Test FaissStore.load and FaissStore.save", async () => {
    ...
    const loadedVectorStore = await FaissStore.load(tempDirectory, new OpenAIEmbeddings());
...
test("Test FaissStore.loadFromPython", async () => {
    ...
    const loadedFromPythonVectorStore = await FaissStore.loadFromPython(path.join(__dirname, "faiss.int.test.data/faiss_index"), new OpenAIEmbeddings());
    expect(loadedFromPythonVectorStore.index?.ntotal()).toBe(42);
    const results0 = await loadedFromPythonVectorStore.similaritySearch("What did the president say about Ketanji Brown Jackson");
...
And I did that 4 days ago, when I nominated Circuit Court of Appeals Judge Ketanji Brown Jackson. One of our nation’s top legal minds, who will continue Justice Breyer’s legacy of excellence.`,
    ...
    expect(results2[0]).toEqual(new Document({
        pageContent: "おはようございます。",
        metadata: { source: "addDocuments_2" },
    }));

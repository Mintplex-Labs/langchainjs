...
test("Test FaissStore.fromTexts + addVectors", async () => {
    const vectorStore = await FaissStore.fromTexts(["Hello world"], [{ id: 2 }], new FakeEmbeddings());
...
test("Test FaissStore Exceptions", async () => {
    ...
    expect(() => vectorStore.index).toThrow("Vector store not initialised yet. Try calling `fromTexts` or `fromDocuments` first.");
    await vectorStore.addVectors([[1, 1]], [
        new Document({
            pageContent: "our world",
            metadata: { id: 8 },
        }),
    ...
    await expect(async () => {
        await vectorStore.addVectors([[1, 1, 1]], [
            new Document({
                pageContent: "our world",
                metadata: { id: 8 },
            }),
        ]);
    }).rejects.toThrow("Vectors must have the same length as the number of dimensions (2)");
    await expect(async () => {
        await vectorStore.similaritySearchVectorWithScore([1, 1, 1], 1);
    }).rejects.toThrow("Query vector must have the same length as the number of dimensions (2)");
    await expect(async () => {

...
test.skip("Test generative agent end-to-end", async () => {
    const Simulation = async () => {
        ...
        // Tommie
        const tommiesMemory = new GenerativeAgentMemory(llm, await createNewMemoryRetriever(), { reflectionThreshold: 8 });
        const tommie = new GenerativeAgent(llm, tommiesMemory, {
            name: "Tommie",
            age: 25,
        ...
        */
        const interviewAgent = async (agent, message) => {
        ...
        */
        ...
        console.log(await interviewAgent(tommie, "Tell me about how your day has been going"));
        /*
        ...
        */
        console.log(await interviewAgent(tommie, "Tell me about your childhood dog!"));
        /*
        ...
        */
        ...
        const evesMemory = new GenerativeAgentMemory(llm, await createNewMemoryRetriever(), {
            verbose: false,
            reflectionThreshold: 5,
        });
        const eve = new GenerativeAgent(llm, evesMemory, {
            name: "Eve",
            age: 34,
            traits: "curious, helpful",
            status: "just started her new job as a career counselor last week and received her first assignment, a client named Tommie.",
            // dailySummaries: [
        ...
        const eveInitialSummary = await eve.getSummary({
            forceRefresh: true,
        });
        ...
        */
        console.log(await interviewAgent(eve, "Tommie is looking to find a job. What are are some things you'd like to ask him?"));
        /*
        ...
        */
        ...
        const runConversation = async (agents, initialObservation) => {
            // Starts the conversation bewteen two agents
            const [, observation] = await agents[1].generateReaction(initialObservation);
            console.log("Initial reply:", observation);
            // eslint-disable-next-line no-constant-condition
            while (true) {
                let breakDialogue = false;
                for (const agent of agents) {
                    const [stayInDialogue, agentObservation] = await agent.generateDialogueResponse(observation);
        ...
        const agents = [tommie, eve];
        await runConversation(agents, "Tommie said: Hi, Eve. Thanks for agreeing to meet with me today. I have a bunch of questions and am not sure where to start. Maybe you could first share about your experience?");
        /*
        ...
        */
        ...
        const tommieSummary = await tommie.getSummary({
            forceRefresh: true,
        });
        ...
        */
        const eveSummary = await eve.getSummary({ forceRefresh: true });
        ...
        */
        const interviewOne = await interviewAgent(tommie, "How was your conversation with Eve?");
        ...
        */
        const interviewTwo = await interviewAgent(eve, "How was your conversation with Tommie?");
        ...
        */
        const interviewThree = await interviewAgent(eve, "What do you wish you would have said to Tommie?");

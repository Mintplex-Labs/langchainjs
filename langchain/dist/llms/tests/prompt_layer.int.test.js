...
test("Test PromptLayerOpenAI returns promptLayerID if returnPromptLayerId=true", async () => {
    ...
    expect(typeof res.generations[0][0].generationInfo?.promptLayerRequestId).toBe("number");
    const modelB = new PromptLayerOpenAI({
        maxTokens: 5,
        modelName: "text-ada-001",
    });
    const resB = await modelB.generate(["Print hello world"]);
    expect(resB.generations[0][0].generationInfo?.promptLayerRequestId).toBeUndefined();
});
...
test("Test PromptLayerChatOpenAI returns promptLayerID if returnPromptLayerId=true", async () => {
    ...
    expect(typeof respA.generations[0][0].generationInfo?.promptLayerRequestId).toBe("number");
    ...
    expect(respB.generations[0][0].generationInfo?.promptLayerRequestId).toBeUndefined();
});

...
test("Run agent with klarna and requests tools", async () => {
    ...
    const agent = await initializeAgentExecutorWithOptions(tools, new ChatOpenAI({ temperature: 0 }), { agentType: "chat-zero-shot-react-description", verbose: true });
...
test("Run agent with incorrect api key should throw error", async () => {
    ...
    await expect(() => model.call([new HumanChatMessage(input)])).rejects.toThrowError("Request failed with status code 401");
    // Test that the agent throws the same error
    await expect(() => executor.call({ input })).rejects.toThrowError("Request failed with status code 401");
});

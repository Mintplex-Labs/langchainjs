...
test("Can parse JSON with text in front of it", async () => {
    const testCases = [
        ...
        {
            input: '{"action":"ToolWithJson","action_input":"The tool input ```json\\n{\\"yes\\":true}\\n```"}',
    ...
    const p = ChatConversationalAgentOutputParserWithRetries.fromLLM(new ChatOpenAI({ temperature: 0, modelName: "gpt-3.5-turbo" }), {
    ...
    for (const message of testCases) {
        ...
        if (message.tool === "Final Answer") {
            ...
            if (message.type === "stringified_object") {
                expect(JSON.parse(message.toolInput)).toStrictEqual(JSON.parse(parsed.toolInput));
            }
            else {

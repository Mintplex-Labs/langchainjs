...
test("JsonListKeysTool", async () => {
    ...
    expect(await jsonListKeysTool.call("/baz/test/foo")).toContain("not a dictionary");
    expect(await jsonListKeysTool.call("/baz/test/foo/0")).toContain("not a dictionary");
    expect(await jsonListKeysTool.call("/baz/test/qux")).toContain("not a dictionary");
...
test("JsonListKeysTool, paths containing escaped characters", async () => {
    ...
    expect(await jsonListKeyTool.call("/paths")).toBe("a~0b, a~1b, a~0~1b, a~1~1~0b");
});
test("JsonGetValueTool", async () => {
    ...
    expect(await jsonGetValueTool.call("")).toBe(`{"foo":"bar","baz":{"test":{"foo":[1,2,3],"qux":[{"x":1,"y":2,"z":3},{"a":1}]}}}`);
    expect(await jsonGetValueTool.call("/foo")).toBe("bar");
    expect(await jsonGetValueTool.call("/baz")).toBe(`{"test":{"foo":[1,2,3],"qux":[{"x":1,"y":2,"z":3},{"a":1}]}}`);
    expect(await jsonGetValueTool.call("/baz/test")).toBe(`{"foo":[1,2,3],"qux":[{"x":1,"y":2,"z":3},{"a":1}]}`);
    expect(await jsonGetValueTool.call("/baz/test/foo")).toBe("[1,2,3]");
    expect(await jsonGetValueTool.call("/baz/test/foo/0")).toBe("1");
    expect(await jsonGetValueTool.call("/baz/test/qux")).toBe(`[{"x":1,"y":2,"z":3},{"a":1}]`);
    expect(await jsonGetValueTool.call("/baz/test/qux/0")).toBe(`{"x":1,"y":2,"z":3}`);
...
test("JsonGetValueTool, large values", async () => {
    ...
    expect(await jsonGetValueTool.call("/baz/test")).toContain("large dictionary");
...
test("JsonGetValueTool, paths containing escaped characters", async () => {
    ...
    const jsonGetValueTool = new JsonGetValueTool(jsonSpec);
    expect(await jsonGetValueTool.call("/paths/~0IDSGenericFXCrossRate")).toBe("1");
    expect(await jsonGetValueTool.call("/paths/~1IDSGenericFXCrossRate")).toBe("2");
    expect(await jsonGetValueTool.call("/paths/~0~1IDSGenericFXCrossRate")).toBe("3");
    expect(await jsonGetValueTool.call("/paths/~1~0IDSGenericFXCrossRate")).toBe("4");
});
